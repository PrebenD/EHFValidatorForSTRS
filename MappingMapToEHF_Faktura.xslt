...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
		<xsl:variable name="vrtfPass1Result">
			<xsl:variable name="var1_Invoice" select="//ns0:Interchange/ns0:Invoice"/>
				...
				<cac:AccountingCustomerParty>
					<cac:Party>
						...
						<cac:Contact>
							<cbc:ID>
								<xsl:for-each
									select="$var1_Invoice/ns0:InvoiceHeader/ns0:Buyer/ns0:ContactPerson">
									<xsl:value-of select="string(ns0:Name)"/>
								</xsl:for-each>
							</cbc:ID>
							<xsl:for-each
				...
				<xsl:for-each select="$var1_Invoice">
					...
					<xsl:for-each select="ns0:InvoiceDiscountChargesAndTax/ns0:InvoiceDiscount">
						...
						<cac:AllowanceCharge>
							...
							<xsl:for-each
								select="$var26_cur/ns0:InvoiceSummary/ns0:VatTotalsInfo/ns0:VatPercent">
								<xsl:variable name="var49_resultof_cast" select="number(string(.))"/>
								<xsl:if
							...
							<xsl:for-each
								select="$var26_cur/ns0:InvoiceSummary/ns0:VatTotalsInfo/ns0:VatPercent[(string(number(string(.))) = '0.00')]">
								<cac:TaxCategory>
					...
					<xsl:for-each select="ns0:InvoiceDiscountChargesAndTax/ns0:InvoiceCharges">
						...
						<cac:AllowanceCharge>
							...
							<xsl:for-each select="$var27_Amount">
								<xsl:variable name="var28_true">
									<xsl:choose>
										<xsl:when test="$var27_Amount[(number(string(.)) &gt; '0')]">
							...
							<xsl:for-each
								select="$var29_cur/ns0:InvoiceSummary/ns0:VatTotalsInfo/ns0:VatPercent">
								<xsl:variable name="var49_resultof_cast" select="number(string(.))"/>
								<xsl:if
									test="string((($var49_resultof_cast &gt;= '5') and ($var49_resultof_cast &lt;= '18'))) != 'false'">
									<cac:TaxCategory>
							...
							<xsl:for-each
								select="$var29_cur/ns0:InvoiceSummary/ns0:VatTotalsInfo/ns0:VatPercent[(string(number(string(.))) = '0')]">
								<cac:TaxCategory>
									<cbc:ID schemeID="UNCL5305">E</cbc:ID>
									<cac:TaxScheme>
										<cbc:ID>VAT</cbc:ID>
						...
						<xsl:for-each select="$var30_InvoiceSummary/ns0:VatTotalsInfo">
							...
							<cac:TaxSubtotal>
								...
								<cac:TaxCategory>
									...
									<cac:TaxScheme>
										<!--<xsl:if
											test="$var30_InvoiceSummary/ns0:InvoiceTotals/ns0:VatTotalsAmount">-->
										<cbc:ID>VAT</cbc:ID>
										<!--</xsl:if>-->
									</cac:TaxScheme>
								</cac:TaxCategory>
				...
				<cac:LegalMonetaryTotal>
					...
					<xsl:for-each select="$var1_Invoice">
						<cbc:TaxExclusiveAmount>
							...
							/>-->
							<xsl:value-of
								select="ns0:InvoiceSummary/ns0:InvoiceTotals/ns0:NetAmount"/>
						</cbc:TaxExclusiveAmount>
					</xsl:for-each>
					<xsl:for-each select="$var1_Invoice">
						<cbc:TaxInclusiveAmount>
							...
							<!--<xsl:value-of
								select="string(number(string(ns0:InvoiceSummary/ns0:InvoiceTotals/ns0:GrossAmount)))"
							/>-->
							<xsl:value-of
								select="ns0:InvoiceSummary/ns0:InvoiceTotals/ns0:ActualPayment"/>
						</cbc:TaxInclusiveAmount>
					</xsl:for-each>
					<xsl:for-each select="$var1_Invoice">
					...
					<xsl:for-each select="$var1_Invoice">
						...
						<xsl:for-each
							select="ns0:InvoiceSummary/ns0:InvoiceTotals/ns0:ActualPayment">
							<xsl:variable name="var39_Currency" select="@Currency"/>
							<cbc:PayableAmount>
								<xsl:attribute name="currencyID" namespace="">
									<xsl:value-of
										select="string($var38_cur/ns0:InvoiceHeader/ns0:Payment/ns0:Currency)"
					...
					<xsl:for-each select="$var40_BaseItemDetails">
						<cac:InvoiceLine>
							...
							<xsl:for-each select="ns0:QuantityInvoiced">
								<cbc:InvoicedQuantity unitCodeListID="UNECERec20" unitCode="PK">
							...
							<cac:TaxTotal>
								...
								<cbc:TaxAmount>
									...
									<xsl:choose>
										<xsl:when test="ns0:VatInfo/ns0:VatAmount">
											<xsl:value-of
												select="ns0:VatInfo/ns0:VatAmount[string(number(string(.)))]"
											/>
										</xsl:when>
							...
							<cac:Item>
								...
								<cac:ClassifiedTaxCategory>
									<xsl:choose>
										<xsl:when test="ns0:VatInfo/ns0:VatPercent">
											<xsl:for-each
											...
											<xsl:for-each select="ns0:VatInfo/ns0:VatPercent">
												<cbc:Percent>
												<!--<xsl:value-of select="ns0:VatInfo/ns0:VatPercent"/>-->
